...
def test_print_paths_preserves_existing_lines():
  ...
  try:
    ...
    # Check for header
    assert "=== Python Entry Point Paths ===" in result.stdout, f"Expected header in output, got: {result.stdout}"

    # Check for PYTHON_CMD
    assert "PYTHON_CMD:" in result.stdout, f"Expected 'PYTHON_CMD:' in output, got: {result.stdout}"
    ...
    assert "ROS_DISTRO:" in result.stdout, f"Expected 'ROS_DISTRO:' in output, got: {result.stdout}"
    ...
    # Check for footer
    assert "================================" in result.stdout, f"Expected footer in output, got: {result.stdout}"
...
def test_print_paths_preserves_package_import_checks():
  ...
  try:
    ...
    assert "onnxruntime" in result.stdout, f"Expected 'onnxruntime' in output, got: {result.stdout}"
    assert "numpy" in result.stdout, f"Expected 'numpy' in output, got: {result.stdout}"
    assert "yaml" in result.stdout, f"Expected 'yaml' in output, got: {result.stdout}"
