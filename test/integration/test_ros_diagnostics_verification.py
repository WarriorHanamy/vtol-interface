...
def test_print_paths_includes_ros_lines_and_package_imports():
  ...
  try:
    ...
    assert "humble" in result.stdout, f"Expected 'humble' in output, got: {result.stdout}"
    ...
    assert "ROS_BOOTSTRAP:" in result.stdout, f"Expected 'ROS_BOOTSTRAP:' in output, got: {result.stdout}"
