...
def test_missing_setup_file_with_custom_distro():
  ...
  try:
    ...
    if result.returncode != 0:
      assert len(result.stderr) > 0, "Expected error message in stderr"
      assert (
        "setup.bash" in result.stderr.lower() or "not found" in result.stderr.lower() or "foxy" in result.stderr.lower()
...
def test_suggest_ros_distro_in_error():
  ...
  try:
    ...
    if result.returncode != 0:
      ...
      assert "setup.bash" in error_output, f"Expected error message to mention setup.bash, got: {error_output}"
  finally:
    os.unlink(test_script)
