...
from infer_utils.actors import BasePolicyActor
...
class TensorRTPolicyActor(BasePolicyActor):
    ...
    def __init__(self, engine_path: Path, node_logger=None,
                 max_batch_size: int = 1, enable_fp16: bool = True):
        """
        ...
        if not TRT_AVAILABLE:
            raise ImportError("TensorRT is not available. Install with: pip install nvidia-tensorrt")
    ...
    def _copy_output_from_device(self, buffers: List[Dict]) -> Dict[str, np.ndarray]:
        ...
        for buffer in buffers:
            ...
            is_output = self._engine.get_tensor_mode(tensor_name) == trt.TensorIOMode.OUTPUT
...
class TensorRTGRUActor(TensorRTPolicyActor):
    ...
    def __init__(self, engine_path: Path, hidden_dim: int = 64,
                 num_layers: int = 1, node_logger=None,
        ...
        if CUDA_AVAILABLE:
            self._h_state_device = cuda.mem_alloc(
                self.h_state.nbytes
            )
