...
class ONNXProvider(InferenceProvider):
    ...
    def __init__(self, onnx_path: Path, actor_type: str = "mlp",
...
class TensorRTProvider(InferenceProvider):
    ...
    def __init__(self, engine_path: Path, actor_type: str = "mlp",
                 hidden_dim: int = 64, num_layers: int = 1,
                 expected_input_shape: Optional[Tuple[int, ...]] = None,
                 expected_output_shape: Optional[Tuple[int, ...]] = None,
                 node_logger=None):
        ...
        try:
            from infer_utils.tensorrt_actor import TensorRTGRUActor, TensorRTMLPActor
