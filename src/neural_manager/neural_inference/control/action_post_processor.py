...
class ActionPostProcessor:
  ...
  def __init__(
    ...
    if self._control_mode not in ["throttle_rates", "acc_rates"]:
      raise ValueError(f"Invalid control_mode '{control_mode}'. Must be 'throttle_rates' or 'acc_rates'")
    ...
    if ros_node is not None:
      ...
      self._throttle_pub = ros_node.create_publisher(Float32, "/neural/throttle_command", 10)
      if self._logger:
        ...
        self._logger.info("📡 动作后处理器话题: /neural/throttle_command (throttle)")
  ...
  def _create_rates_throttle_message(self, throttle_raw: float, rate_frd: np.ndarray) -> VehicleRatesSetpoint:
    """
    ...
    msg.thrust_body[2] = float(throttle)  # Negative throttle demand (NED Z-axis down)
  ...
  def _create_rates_acc_message(self, thrust_acc_raw: float, rate_frd: np.ndarray) -> VehicleThrustAccSetpoint:
    """
  ...
  def print_control_command(self, action_display_dict: dict, prefix: str = ""):
    ...
    if control_mode == "throttle_rates":
      ...
      print(f"{prefix}🎮 控制指令 #{self._control_command_count} (4维) [Throttle模式]:")
      throttle = action_display_dict["throttle"]
      throttle_raw = action_display_dict["throttle_raw"]
      print(f"{prefix}  ⬆️  油门:           {throttle:.3f} (归一化, 原始: {throttle_raw:.3f})")
