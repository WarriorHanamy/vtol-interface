...
class BuilderConfig:
  ...
  @classmethod
  def int8(cls, calibrator: Any | None = None) -> "BuilderConfig":
...
def build_tensorrt_engine(
  ...
  try:
    ...
    _log(log_func, "  ONNX model parsed successfully")
  ...
  if check_hash:
    ...
    if meta_path.exists():
      ...
      if stored_onnx_hash == onnx_hash:
        ...
        _log(log_func, "  Engine outdated (ONNX model changed)")
        return False
    else:
      # No metadata file, assume engine is outdated
      _log(log_func, "  Engine metadata not found")
      return False
